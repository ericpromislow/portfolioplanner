...
describe 'Analyze' do
  ...
  describe 'ignore holdings in the TRADES section' do
    before do
    ...
    describe 'options' do
      ...
      it 'ignores the options and weights everything else' do
        summary = @analyzer.get_summary()
        holdings_by_category = summary[:holdings_by_category]
        expect(holdings_by_category.size).to eq(summary[:adjustments_by_category].size)
        expected_data = {
          :USEq=> {
              :SLV=>{:value=>25.00, :weight=>25.0, :delta=>0},
              :VOO=>{:value=>75.00, :weight=>75.0, :delta=>0},
          },
          :CASH => {
              :cash=>{:value=>100.00, :weight=>100.0, :delta=>0},
            },
          :TRADES => {
              "TSLA 091820 420 C"=>{:value=>405.00,},
            },
        }
        expected_data.each do |category, holdings|
        ...
        trades = holdings_by_category[:TRADES]
        expect(trades.size).to eq(1)
        expect(trades.keys[0]).to eq(:"TSLA 091820 420 C")
        val = trades[:"TSLA 091820 420 C"]
        expect(val.keys.size).to eq(2)
        expect(val[:value]).to eq(405.00)
        expect(val[:weight]).to eq(0)
      end
